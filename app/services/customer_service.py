...
class CustomerService(ICustomerService):
    ...
    async def create_customer_with_orders(self, command: CustomerCreateCommand) -> None:
        async with self.uow:
            customer_dto = CustomerCreateDTO(first_name=command.first_name, last_name=command.last_name)
            customer = await self.uow.customers.create(customer_dto)

            for order in command.orders:
                order_dto = OrderCreateDTO(customer_id=customer.id, name=order.name, price=order.price)
